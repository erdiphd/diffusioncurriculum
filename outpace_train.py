...
subprocess.check_call(['pip', 'install', 'gym==0.22.0'])
...
from diffusion.diffusion_goal_sampler import DiffusionGoalSampler
import threading
...
class UniformFeasibleGoalSampler:
    def __init__(self, env_name):        
        ...
        elif env_name in ["PointLongCorridor-v0"]:
            self.LOWER_CONTEXT_BOUNDS = np.array([-2, -2])
            self.UPPER_CONTEXT_BOUNDS = np.array([26, 14])
        else:
...
class Workspace(object):
    def __init__(self, cfg):
        ...
        if cfg.env in ['sawyer_peg_push', 'sawyer_peg_pick_and_place']:
            ...
            elif cfg.env in ["PointUMaze-v0", "PointSpiralMaze-v0", "PointNMaze-v0", "PointLongCorridor-v0"]:
                from gym.wrappers.time_limit import TimeLimit
                import mujoco_maze                                             
        ...
        if cfg.use_hgg:
            ...
            self.diffusion_achieved_trajectory_pool = TrajectoryPool(**cfg.hgg_kwargs.trajectory_pool_kwargs)
        ...
        for r in range(repeat):            
            ...
            for episode in range(self.cfg.num_eval_episodes):
                ...
                if self.cfg.trajectory_save:
                    np.save(self.trajectory_dir_test + "/" + str(self.step) + "_ep_" + str(episode) + ".npy",np.array(numpy_array_container))
                numpy_array_container = []
    ...
    def _run(self):        
        ...
        if True: #TODO
            if self.cfg.env in ['AntMazeSmall-v0', "PointUMaze-v0"]:  
                ...
                final_goal_states += np.random.normal(loc=np.zeros_like(final_goal_states), scale=0.5*np.ones_like(final_goal_states))
            elif self.cfg.env  == "PointLongCorridor-v0":
                final_goal_states = np.tile(np.array([24., 0.]), (self.cfg.aim_num_precollect_init_state,1))
                final_goal_states += np.random.normal(loc=np.zeros_like(final_goal_states), scale=0.5*np.ones_like(final_goal_states))
        ...
        if self.cfg.diffusion_kwargs.diffusion:
            if self.cfg.env in ['AntMazeSmall-v0', "PointUMaze-v0"]:
                max_action = [10,10]
                min_action = [-2, -2]
            elif self.cfg.env  == "PointSpiralMaze-v0":
            ...
            self.diffusion_model = DiffusionGoalSampler(state_dim=self.env.obs_dim, action_dim=self.env.goal_dim, max_action = max_action, min_action = min_action, device=self.device, agent=agent, diffusion_configuration=self.cfg.diffusion_kwargs)
        current_pocket_success = 0
        current_pocket_trial = 0

        if self.cfg.render_env_training or self.cfg.render_env_evaluaton:
            def continuous_run():
                render_env = []
                if self.cfg.render_env_training:
                    render_env.append(self.env)
                if self.cfg.render_env_evaluaton:
                    render_env.append(self.eval_env)
                else:
                    raise NotImplementedError
                while True:
                    for _env in render_env:
                        _env.render()
        ...
        if self.cfg.trajectory_save:
            ...
            self.trajectory_dir = utils.make_dir(self.work_dir, 'trajectory')
            self.trajectory_dir_test = utils.make_dir(self.work_dir, 'trajectory_test')
        while self.step <= self.cfg.num_train_steps:
            if done:
                if self.step > 0:
                    ...
                    # diffusion_flag
                    if self.cfg.diffusion_kwargs.diffusion:
                        if episode % self.cfg.diffusion_kwargs.train_diffusion_per_epoch == 0:
                            self.diffusion_model.train(self.get_buffer(), self.env, self.diffusion_achieved_trajectory_pool)
                    ...
                    if self.cfg.use_hgg :
                        if episode % self.cfg.hgg_kwargs.hgg_sampler_update_frequency ==0 :
                            ...
                            #hgg_sampler.update(initial_goals, final_goal_states, replay_buffer = self.expl_buffer, meta_nml_epoch=episode, outpace_train = self)
                ...
                if self.cfg.use_hgg:
                    ...
                    while True:
                        ...
                        #sampled_goal = self.diffusion_model.sample_goal(torch.Tensor(obs[:self.env.obs_dim]).to(self.device).reshape(1,-1)).detach().cpu().numpy().reshape(-1)
                        hgg_sampler.update(final_goal_states, replay_buffer=self.expl_buffer,meta_nml_epoch=episode, outpace_train=self)
                        sampled_goal = hgg_sampler.sample(episode % 100)
                        print(sampled_goal)
                        # sampled_goal = np.array([0,8])
                ...
                generated_curriculum_points.append(sampled_goal)
                ...
                if episode % self.cfg.train_episode_video_freq == 0 or episode in [25, 50, 75, 100]:
                    ...
                    for k in range(visualize_num_iter + 1):
                        ...
                        if self.cfg.use_aim:
                            visualize_discriminator_matplot(
                                normalizer=agent.normalize_obs if self.cfg.normalize_f_obs else None,
                                ...
                                curriculum_goal = sampled_goal
                                )
                            #
                            # visualize_discriminator2(
                            #     normalizer=agent.normalize_obs if self.cfg.normalize_f_obs else None,
                            #     discriminator=agent.aim_discriminator,
                            #     env_name=self.cfg.env,
                            #     aim_input_type=self.cfg.aim_kwargs.aim_input_type,
                            #     device=self.device,
                            #     savedir_w_name=self.train_video_recorder.save_dir + '/aim_f_visualize_train_goalfix_' + str(
                            #         episode) + '_s' + str(k),
                            #     )
                episode_reward = 0
                episode_step = 0
            ...
            if last_timestep:
                ...
                if self.cfg.use_hgg:
                    ...
                    if 'aim_f' in self.hgg_sampler.cost_type or 'meta_nml' in self.hgg_sampler.cost_type:
                        temp_episode_init =temp_episode_observes[0] # for bias computing
                    else:
                        raise NotImplementedError


                    for k in range(len(temp_episode_observes)):
                        temp_episode_ag.append(temp_episode_observes[k])
                    ...
                    achieved_trajectories_diffusion = [np.hstack((np.array(temp_episode_ag)[1:,:], np.array(episode_actions)))]
                    ...
                    for i in range(len(achieved_trajectories)):
                        ...
                        if self.cfg.env in ['AntMazeSmall-v0', "PointUMaze-v0", "PointSpiralMaze-v0", "PointNMaze-v0"]:
                            ...
                            raise NotImplementedError
                        # if goal_distance(achieved_trajectories[i][0][-4:-2], achieved_trajectories[i][-1][-4:-2])>threshold: # if there is a difference btw first and last timestep ?
                        if (np.std(np.array(achieved_trajectories[i][:][-3]))) > 3 and (np.std(np.array(achieved_trajectories[i][:][-4]))) > 3:
                    ...
                    for idx in selection_trajectory_idx.keys():
                        ...
                        self.diffusion_achieved_trajectory_pool.insert(achieved_trajectories_diffusion[idx].copy(), achieved_init_states[idx].copy())
            ...
            if self.cfg.use_residual_randomwalk:
                if self.env.is_residual_goal:
                    if (self.env.residual_goalstep % 10 == 0) or info.get('is_current_goal_success'):
                        if (self.cfg.use_uncertainty_for_randomwalk not in [None, 'none', 'None']) and self.step > 2048: #TODO
                            residual_goal = self.get_agent().sample_randomwalk_goals(obs = obs, ag = self.env.convert_obs_to_dict(obs)['achieved_goal'], \
                                ...
                                diffusion_model_sample_goal = self.diffusion_model.sample_goal)
                        else:
                        ...
                        generated_curriculum_points.append(residual_goal)
                else:
