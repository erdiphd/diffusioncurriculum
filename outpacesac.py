...
class OUTPACEAgent(object):
    ...
    def normalize_obs(self, obs, env_name, model=None):
        # normalize to [-1,1]
        if obs is None:
            return None
        if type(obs)==np.ndarray:
            obs = obs.copy()
        elif type(obs)==torch.Tensor and model=="diffusion":
            obs = obs
        ...
        if env_name in ['AntMazeSmall-v0', "PointUMaze-v0"]:
            center, scale = 4, 6
        elif env_name in ["PointSpiralMaze-v0", "PointLongCorridor-v0"]: #TODO
            center, scale = 0, 10
        ...
        if obs.shape[-1]==self.goal_dim or obs.shape[-1]==self.goal_dim*2:
            ...
            dim = obs.shape[-1]
            center_torch = np.zeros(dim)
            center_torch[:2] = center
            center_torch[-4:] = center
            scale_torch = np.ones(dim)
            scale_torch[:2] = scale
            scale_torch[-4:] = scale
            return torch.clip((obs - torch.Tensor(center_torch).to(self.device)) / torch.Tensor(scale_torch).to(self.device),-100, 100)
        else:
            obs[..., :2] = (obs[..., :2]-center)/scale
            obs[..., -4:] = (obs[..., -4:]-center)/scale
    ...
    def sample_randomwalk_goals(self, obs, ag, episode, env, replay_buffer, num_candidate = 5, random_noise = 2.5, uncertainty_mode = 'f',diffusion_model_sample_goal=None):
        noise = np.random.uniform(low=-random_noise, high=random_noise, size=(num_candidate, env.goal_dim))
        obs = diffusion_model_sample_goal(torch.Tensor(obs[:env.obs_dim]).to(self.device).reshape(1, -1)).detach().cpu().numpy() + noise[0,:]
